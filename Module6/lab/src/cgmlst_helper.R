...
# helper func for plot  loci_completeness using stack bars
plot_loci_completeness <- function(data) {
  data_plot <- data %>%
    arrange(desc(valid_alleles)) %>%
    mutate(newcol = 25 * (rep(1:ceiling(nrow(.)/25), each = 25)[1:nrow(.)])) %>%
    select(newcol, valid_alleles, missing_alleles) %>%
    pivot_longer(!newcol, names_to = "variable", values_to = "Allele") %>%
    group_by(newcol, variable) %>%
    summarise(Count = sum(Allele) / 25, .groups = "drop")
  
## scale up the last bar from 1326 to 1343
  scale_factor = (1343-1325)/25
  data_plot <-  data_plot %>%
                mutate(Count = case_when(newcol == "1350" ~ Count/scale_factor,
                                         T ~ Count))
  
  # Plotting
  ggplot(data_plot, aes(x = newcol, y = Count, fill = variable)) +
    geom_bar(stat = "identity", position = "stack") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    scale_fill_manual(values = c("valid_alleles" = "blue", "missing_alleles" = "red")) + # Assign colors manually
    scale_x_continuous(breaks = c(seq(50, nrow(data), 50), 1343)) +
    scale_y_continuous(breaks = c(seq(0, 350, 50), 402)) +
    labs(x = "Locus (n = 1343)", y = "Genome count (n = 402)", fill = "Allele")
}
